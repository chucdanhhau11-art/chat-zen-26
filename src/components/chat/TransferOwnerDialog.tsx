  ...
  return (
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="sm:max-w-md">
        ...
        {confirming && keepMembership ? (
          <p className="text-sm text-muted-foreground py-2">
            Xác nhận nhường quyền nhóm trưởng cho{' '}
            <span className="font-semibold text-foreground">{selectedMember?.profile?.display_name || 'thành viên này'}</span>? Hành động này không thể hoàn tác bởi bạn.
          </p>
        ) : (
        ...
        <div className="flex justify-end gap-2 pt-2">
          <Button variant="outline" onClick={() => (confirming ? setConfirming(false) : onClose())} disabled={loading}>
            {confirming ? 'Quay lại' : 'Huỷ'}
          </Button>
          <Button onClick={handleConfirm} disabled={!selected || loading}>
            {loading ? 'Đang xử lý...' : keepMembership ? (confirming ? 'Xác nhận nhường quyền' : 'Nhường quyền') : 'Chuyển & Rời nhóm'}
          </Button>
        </div>
