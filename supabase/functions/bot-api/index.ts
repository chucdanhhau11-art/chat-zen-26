...
serve(async (req) => {
  ...
  try {
    ...
    // ==================== clientInlineQuery ====================
    // Client-friendly endpoint that uses bot_profile_id instead of bot_token
    if (action === "clientInlineQuery") {
      const { bot_profile_id, query, user_id, chat_id } = body;
      if (!bot_profile_id) {
        return new Response(JSON.stringify({ error: "bot_profile_id is required" }), {
      ...
      // Find bot by profile_id
      const { data: clientBot } = await supabase
        .from("bots")
        .select("id, bot_token, webhook_url")
        .eq("profile_id", bot_profile_id)
        .eq("status", "active")
        .maybeSingle();
      ...
      if (clientBot.webhook_url) {
        try {
          ...
          // Webhook failed, use cached results
        }
      }

      // Create bot_event for polling-based bots
      await supabase.from("bot_events").insert({
        bot_id: clientBot.id,
        event_type: "inline_query",
        payload: { query, user_id, chat_id },
